...
def main():
    dimension_boundaries = json.load(open('flight_prediction/grid/DimensionBoundaries.json'))
    ...
    heading_min = dimension_boundaries['heading_min']
    heading_max = dimension_boundaries['heading_max']
    heading_step = dimension_boundaries['heading_step']

    lat_bounds = (lat_min, lat_max, lat_step)
    long_bounds = (lon_min, lon_max, lon_step)
    alt_bounds = (alt_min, alt_max, alt_step)
    heading_bounds = (heading_min, heading_max, heading_step)
    ...
    output_file = "/anvil/projects/tdm/corporate/sandia-trajectory/data/grid_pickle_files/grid_lat_long_alt_heading_full.pkl"

    columns = ['LATITUDE', 'LONGITUDE', 'ALTITUDE', 'HEADING']
    ...
    grid = common.generate_grid_multiprocessing(
        dir_path, [lat_bounds, long_bounds, alt_bounds, heading_bounds], columns, target_column, 128)
